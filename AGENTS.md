...
- Issue 着手時に、どの実行モード(Approval-driven / Goal-driven / Budget-driven)で進めるかを**選択式で確認**する(ユーザー入力を最小化。Claude Code では AskUserQuestion を使う)。🟢 Goal-driven を選んだら続けてゴール(完了条件)を、🟡 Budget-driven を選んだら続けて時間/回数の上限を尋ねる。定義は「AI エージェントの作業規律」の実行モード表。
...
**Issue 着手時に、どのモードで進めるかを最初に確認・合意する。** 全モード共通で、検証 2 段(`just check` + verifier)と「main は PR 経由・squash」(G-0001)は不変。

| 項目 | 🔵 Approval-driven(承認駆動・既定) | 🟢 Goal-driven(ゴール駆動) | 🟡 Budget-driven(予算駆動・時間/回数) |
|---|---|---|---|
| 発動 | 既定 / 「承認駆動で」 | 明確な完了条件 +「ゴールまで自動で」 | 「30分 / 5 コミット 等で区切って自動で」 |
| 停止条件 | 各非自明ステップで承認 | 完了条件(機械検証可能)に到達 | 指定の時間 or 回数の上限に到達 |
| コミット | `just check` green 後、**コミット前に停止**して報告 + 変更確認依頼 → 承認後 | `just check` green 後、こまめに自動コミット | `just check` green 後、こまめに自動コミット |
| 計画 | 着手前に計画 → 承認 | ゴールを最初に合意すれば逐次承認不要 | ゴール曖昧可・上限内でベストエフォート・分岐は仮決めし報告で説明 |

自動モード(🟢🟡)共通のガードレール:

- 完了/上限のほか、**不可逆・外向き操作(PR マージ・ブランチ削除・外部公開)、設計の重大分岐、反復するエラー**では停止して確認する(auto ≠ 無条件で進む)。
- **純ロジックのみ**。音声・TCC・ハードウェア依存は自動不可 → Approval-driven へフォールバック(G-0004 / domain-pitfalls #8)。
