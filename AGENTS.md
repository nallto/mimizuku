...
| 報告待ちの通知(知識本体ではない) | `local/agent-artifacts/report-queue/`(コミットしない。パスの解決基準はG-0010 決定10) |
...
- 多エージェントのオーケストレーション(Workflow等)は、共通skillが手順として定義したものだけを使う。手順本文は`.agents/skills/`、オーケストレーションのスクリプトは製品別アダプターへ置き、その機構を持たないAgentは同じ本文を逐次実行して同一の出力形式を満たす。オーケストレーションを使うskillは、目的、停止条件、1回の起動で使うエージェント数の上限をSKILL.mdに明記する(G-0010。`.claude/skills/`をポインタに限る原則に対する唯一の例外)。
- 並列実行や無人実行で複数の結果が同時に出た場合は、全件を報告キューへ積み、優先度順に1ターン1件だけ報告する。恒久的な知識は報告前にIssue・docs・ADRへ書く。手順の正典は`.agents/skills/report-queue/SKILL.md`。

## コミットと PR

- squash 後に main に残るのは「PR タイトル + squash 本文」。squash 本文は **PR 本文全文**(#14 スタイル)なので、PR 説明(概要 / 変更内容 / 関連 Issue / 任意で人の介在が必要な項目 / Squash body / チェックリスト)がそのまま履歴に残る。レビューコメント・ブランチ上のコミット列は残らない前提で扱う。
...
- 承認者が応答できない状態で進める場合(定期巡回を含む)は、到達可能な限界まで進めて停止する。無人実行が新たな非自明な変更へ着手することはなく、統合前の承認も省略しない。許可・禁止の境界と、コミット前の変更確認を無人時にどう扱うかは[`docs/development.md`の「無人実行と報告キュー」](./docs/development.md#無人実行と報告キュー)、理由はG-0010に従う。
- 人の介在なしに進められない状態になったら、Issue・PRへ`needs-human`ラベルを付け、本文の「人の介在が必要な項目」節に種類と具体手順を書く(種類の一覧はG-0010)。ラベルを付けただけで手順を書かない状態にしない。
