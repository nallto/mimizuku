...
- **macOS 14.4+ + whisper.cpp** OS-26 制約を外せるが「Apple フレームワーク・同梱モデルなし」の設計目標を捨て、GGML モデル配布と GPU/ANE チューニングの負担が増える。却下。
- **macOS 26 + `DictationTranscriber` フォールバックで旧ハード対応** 列挙できないハードウェアクラスのために品質階層とテスト行列が増える。具体的なユーザー報告が出るまで却下。
