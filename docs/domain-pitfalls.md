...
- **#13 マイク単体モードの AEC は「隠しシステム音声 tap」を要し、システム音声 TCC と結合する(AEC-4 / #64)。** WebRTC AEC3 は near-end(マイク)を消すために far-end 参照(スピーカー出力 = システム音声)を必要とする。そのため「マイクのみ」モードでも回り込みを消すには、録音・文字起こしに出さない参照専用の system tap を裏で起動する。帰結として、**マイク単体モードの初回起動でシステム音声の TCC プロンプトが出る**(ユーザーがマイクだけ選んでいてもシステム音声許可を求められる ―― 用途説明でカバー)。参照 tap が使えない(未許可・沈黙)場合は AEC が打ち消さないだけで、マイク録音・文字起こしは後述の安全弁で実時間継続する(graceful degradation)。診断表示は APM 初期化時点では「参照待ち」とし、対応する render / capture の実給餌開始後だけ「有効」、参照失敗・沈黙時は「AEC なしで継続」とする(#74)。
  ...
  - **給餌スケジューラの「参照未着なら capture を APM に入れない」ゲートで、参照沈黙時にマイクが全損または恒久遅延しうる。** `AecFeedScheduler` はこのゲートを持つ(#63 の 3 パターン分岐対策)。参照 tap が沈黙(IOProc 未発火)するとゲートで capture が永遠に保留され、**マイク発話がほぼ全損する**(セッション末に flush されるだけ)。また各 capture を個別に `renderStartGrace` 後へ解放するだけでは、欠損は避けても全フレームが恒久的に約3秒遅れる。`renderStartGrace`(既定 3 秒 ―― 両方モードの tap 起動遅延 +1.2 秒より十分大きく取り正常時は誤発動しない)を最古フレームが超えた時点で **bypass をセッション中ラッチ**し、保留全件と後続 capture を APM に入れず即時素通しする。遅れて render が来ても途中復帰させない(#70 / #74)。
  - **初回 render 到着後に、参照開始前の capture backlog を APM へ遡及投入してはいけない。** 「参照未着中は保留」だけでは不十分。両方モードはマイク側ルーターが先に起動し、system tap が約 +1.2 秒遅れるため、初回 render 到着時点で対応参照のない capture が大量に溜まっている。状態を active にした直後、その backlog をタイムアウト扱いで APM へ流すと、near-end だけを先に学習して AEC3 の初期収束を崩し、マイク単体モードより回り込みやすくなる。初回 render よりホスト時刻が古い capture は `warmupBypass` として **録音・文字起こしには原音で残すが APM へは入れず**、参照開始後の capture から AEC を開始する。actor への到着順はタスクスケジューリングで入れ替わるため判定に使わない。これはセッション中 bypass ではなく開始時だけのウォームアップであり、診断の「有効」は実際に最初の capture を APM へ入れた後に表示する(#74)。
