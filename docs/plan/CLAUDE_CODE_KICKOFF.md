...
その後、実装計画の **Slice 0** を実行する(リポジトリ setup のため Slice 0 自体は Issue 無しでよい ―― PR 概要に理由を書く):
...
- 1 スライス = 1 Issue = 1 セッション/ブランチ。各セッションは対応する Issue と、そのスライスの受け入れ条件の再読から始める。ブランチは Issue 番号で切る。
