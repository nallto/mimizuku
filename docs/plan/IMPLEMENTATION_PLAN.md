...
- **AEC-4(#64 / #74 / #76、ADR-0014)**: 「マイクのみ」モードでも隠し参照 tap で AEC を効かせる(参照 tap は録音・文字起こしに出さない。両方モードと同一経路で本番 tap を直接起動 ―― 事前プローブは本番 tap を無音化するため禁止。domain-pitfalls #13)。APM処理済みマイクだけを正式音源とし、初回render前・参照復旧中はマイク原音を同長の無音へ置換する。初回render未着または復旧が5秒を超えたらセッション失敗とし、AEC初期化失敗時もrawへフォールバックしない。render一時停止はrecoveringへ移り、参照復帰時に新しい同期epochでAECへ復帰する。状態は参照待ち / 有効 / 復旧中 / 失敗を権限診断画面に表示し、「有効」は最初のcaptureを実際にAPMへ入れた後に更新する。#75で処理前後の抑圧品質を計測し、必要なら復帰条件を追加する。
