...
- **AEC-4(#64)**: 「マイクのみ」モードでも隠し参照 tap で AEC を効かせる(参照 tap は録音・文字起こしに出さない。両方モードと同一経路で本番 tap を直接起動 ―― 事前プローブは本番 tap を無音化するため禁止。domain-pitfalls #13)。参照が使えない・一度も来ない場合は同期層の安全弁(`AecFeedScheduler.renderStartGrace`)がマイクを実時間で素通し解放して欠損を防ぎつつ AEC なしで継続する(graceful degradation。#70 の穴も同時に解消)。状態は権限診断画面に表示。完了で #59 クローズ。
