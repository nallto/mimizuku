...
| **フェーズ 1 — 記録の完成** |  |  |  |  |
| S0 | リポジトリ立ち上げ | — | — | 完了(#6) |
| S1 | マイク → ライブ文字起こし | S0 | — | 完了(#24) |
| S2 | セッション録音 + AudioRouter + メニューバー状態表示 | S1 | D1 | 完了(#55) |
| S3 | システム音声 tap ソース | S2 | — | 完了(#56) |
| S4 | デュアルストリーム + 権限診断 | S3 | — | 完了(#65) |
| AEC-1 | WebRTC APM ベンダリング + ブリッジ + オフライン検証(#61) | S4 | AEC-0 | 完了(#67) |
| AEC-2 | AEC 同期層(フレーマ・リングバッファ・ドリフト計測)(#62) | S4 | AEC-0 | 完了(#68) |
| AEC-3 | AEC ライブ統合(両方モード)(#63) | AEC-1, AEC-2 | — | 完了(#72) |
| AEC-4 | マイクのみモードの参照 tap(#64) | AEC-3 | — | 完了 |
| S5 | 文字起こし永続化 + セッションモデル | S2 | D2 | 完了(#38) |
| **フェーズ 2 — メイン画面** |  |  |  |  |
| S6 | 3 ペイン骨格 | S5 | D3 | 完了(#39) |
| S7 | 再生 | S6 | — | 未着手 |
| S8 | 波形表示 | S7 | D4 | 未着手 |
...
- **決定済み(#29 / ADR-0007)**: 停止後は`meta.json`と整形済み`transcript.json`を正典とし、認識処理中だけ追記型`transcript.journal.jsonl`を使う。Markdown / プレーンテキストは派生エクスポート。SQLiteはS5で導入せず、D5で必要と判断した場合も再構築可能なキャッシュに限定する。
- 正常停止ではSpeechのfinalizeを待ってからジャーナルを整形済みスナップショットへ原子的に反映する。ライブ録音、インポート、再文字起こしは同じパイプラインを使う。
- 整形は認識・保存から分離した差し替え可能な契約とし、初期実装は純ロジック、将来はオンデバイスの自然言語処理やLLMへ拡張可能にする。
...
- UX要件は[`docs/design/main-window.md`](../design/main-window.md)で先に固定する。マイクとシステム音声は共通時間軸の同期2レーン、単一音源は存在するレーンだけを表示する。D4／ADR-0009はこの観察可能な要件を変えず、生成、デシメーション、キャッシュ、描画API、長尺性能を決める。
...
### AEC-0 — エコーキャンセル方式(S4 後)→ ADR-0013(決定済み)

- S4 の実機検証でスピーカー回り込み(二重文字起こし)が確認され、会議用途の必須要件として追加された判断(epic #59)。
- Apple VPIO はシステム全体ダッキングで tap 捕捉と非両立(実測・domain-pitfalls #12)。**WebRTC AEC3 を採用**し、処理後音声を文字起こし・録音の両方に使う(ADR-0006 の native 録音方針をマイクについて修正)。代替案比較・出典・ゲート条件は ADR-0013。
- 注: ADR 番号は D2〜D7 が 0007〜0012 を予約済みのため 0013。
...
3. セッションディレクトリの作成(`$XDG_DATA_HOME/mimizuku/sessions/<timestamp>/`、未設定時は`~/.local/share/mimizuku/sessions/<timestamp>/`、詳細はD1 / ADR-0006)。
...
1. 確定`TranscriptSegment`を、run IDとsequenceを持つ`transcript.journal.jsonl`へwrite-through追記する。末尾の不完全行だけを無視して回復し、途中の破損は明示する。
2. 正常停止を「入力停止→Speech finalizeと結果完了待ち→永続化→整形」に変更する。finalizeは5秒を上限に待ち、失敗・期限超過でも最後の表示内容を黙って捨てず、未完了末尾として保存できるようにする。
3. 認識結果を表示ブロックへ変換する整形契約と既定の純ロジック実装(Core)を追加する。同じトラックの隣接結果を句読点・無音区間・時刻で統合・分割し、トラック帰属・時間範囲・言い切られていない末尾を保持する。将来のオンデバイス自然言語処理やLLM実装を同じ契約へ差し替え可能にする。
4. `SessionStore`(Core): セッションの列挙・読み込み・保存。`meta.json`と`transcript.json`は一時ファイルへの同期後に原子的に置換し、ジャーナルはスナップショット同期後に削除する。メタデータはタイトル・日時・長さ・タグ・ステータス、安定トラックID、入力元の来歴を扱う。
5. タイムスタンプ・話者ラベル付きMarkdown / プレーンテキストのエクスポート。
6. `TranscriptSegment.start/end`の時刻原点(セッション開始基準)をデータ仕様として固定(S7の前提)。

受け入れ: セッション中の`kill -9`で失うのは最大でも現在のvolatile発話のみ。正常停止ではSpeechのfinalizeを待ち、確定可能な末尾を保存する。再起動後にセッションの文字起こしと音声が読め、スナップショット置換後・ジャーナル削除前の異常終了でも重複しない(UIは暫定でよい)。異なるトラックを混ぜず、末尾を欠落させずに区切り直した60分セッションのエクスポートが整形されている。

#### AEC-1〜AEC-4 — エコーキャンセル(WebRTC AEC3、ADR-0013)

S4 で確認したスピーカー回り込み(二重文字起こし)の解消。詳細スコープ・受け入れは各 Issue が正典(epic #59)。

- **AEC-1(#61)**: APM ベンダリング(`scripts/build-webrtc-apm.sh`、ピン留め、成果物は gitignore)+ Obj-C++ ブリッジ + 実録音ペアによる**オフライン実測ゲート**(不合格ならライブ統合に進まず再検討)。
- **AEC-2(#62)**: 同期層。10ms フレーマ・タイムスタンプ付きリングバッファ・ドリフト計測を Core 純ロジックとして実装(CI テスト対象)。AEC-1 と並行可。
- **AEC-3(#63)**: ライブ統合(「両方」モード)。受け入れ: 二重混入が実用上消える / CPU ≤5% / 追加レイテンシ ≤20ms / ヘッドホン時に劣化しない / デバイス切替から回復 / 60 分 soak。
- **AEC-4(#64 / #74 / #76、ADR-0014)**: 「マイクのみ」モードでも隠し参照 tap で AEC を効かせる(参照 tap は録音・文字起こしに出さない。両方モードと同一経路で本番 tap を直接起動 ―― 事前プローブは本番 tap を無音化するため禁止。domain-pitfalls #13)。APM処理済みマイクだけを正式音源とし、初回render前・参照復旧中はマイク原音を同長の無音へ置換する。初回render未着または復旧が期限(当初5秒、ADR-0016 決定11で15秒へ改訂)を超えたらセッション失敗とし、AEC初期化失敗時もrawへフォールバックしない。render一時停止はrecoveringへ移り、参照復帰時に新しい同期epochでAECへ復帰する。状態は参照待ち / 有効 / 復旧中 / 失敗を権限診断画面に表示し、「有効」は最初のcaptureを実際にAPMへ入れた後に更新する。#75で処理前後の抑圧品質を計測し、必要なら復帰条件を追加する。

### フェーズ 2 — メイン画面

UI作業は着手前に`.agents/skills/macos-ui-design/SKILL.md`、[`docs/DESIGN.md`](../DESIGN.md)、対象画面仕様を読む。
...
D4 の決定に従う。音量ベース波形(RMS/ピークのデシメーション)。マイクとシステム音声がある場合はセッション共通時間軸の同期2レーンとし、単一音源では存在するレーンだけを表示する。録音中に逐次計算してキャッシュする設計を軸に検討(ライブ表示にも流用)。`WaveformDecimator` は Core の純ロジックとして CI テスト。再生位置カーソル、クリックシーク。長時間ファイルの全サンプル読み込みは禁止(必ずデシメーション経由)。
...
| JSONスナップショットまたはジャーナルの破損 | 中 | データ読込失敗 | 原子的置換 + 不完全な末尾行だけを無視 + 音声・有効なジャーナルから救済(D2 / ADR-0007) |
