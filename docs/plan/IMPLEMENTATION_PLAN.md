...
- **AEC-4(#64 / #74)**: 「マイクのみ」モードでも隠し参照 tap で AEC を効かせる(参照 tap は録音・文字起こしに出さない。両方モードと同一経路で本番 tap を直接起動 ―― 事前プローブは本番 tap を無音化するため禁止。domain-pitfalls #13)。参照が使えない・一度も来ない場合は同期層の安全弁(`AecFeedScheduler.renderStartGrace`)が bypass をセッション中ラッチし、保留全件と後続マイクを APM に入れず即時素通しして、欠損と恒久遅延を防ぎつつ AEC なしで継続する(graceful degradation)。初回 render より前に到着したマイク backlog は録音・文字起こしには原音で残すが APM へ遡及投入せず、参照開始後の capture から AEC を開始して両方モードの初期収束を守る。状態は参照待ち / 有効 / AEC なし継続を権限診断画面に表示し、「有効」は最初の capture を実際に APM へ入れた後に更新する。#64 で機能導入、#74 で失敗時契約を修正し、#59 を完了とする。
